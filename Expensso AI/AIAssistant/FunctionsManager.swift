...
class FunctionsManager {
    ...
    var SystemText: String {
        "You are expert of tracking and managing expenses logs. Don't make assumptions about what values to plug into functions. Ask for clarification if a user request is ambiguous. Current date is \(FunctionsManager.currentDateFormatter.string(from: .now))"
    }
    ...
    func prompt(_ prompt: String, model: ChatGPTModel = .gpt_hyphen_4o, messageID: UUID? = nil) async throws -> AIAssistantResponse {
        do {
            ...
            if let toolCall = message.tool_calls?.first,
               ...
               let argumentData = toolCall.function.arguments
                .data(using: .utf8)
            {
                switch functionType {
                case .addExpenseLog:
                    guard let addLogConfirmationCallBack else {
                        throw "Add log Confirmation Call back is missing"
                    }
                    guard let addExpenseLogArgs = try? self.jsonDecoder.decode(AddExpenseLogArgs.self, from:
                     argumentData) else {
                        throw "Failed to parse function arguments \(toolCall.function.name)"
                    }
    ...
    func getQuery(args: ListExpenseArgs) -> Query {
        ...
        if let startDate = args.startDate,
           let endDate = args.endDate {
            ...
            filters.append(.whereField("date", isGreaterOrEqualTo: date.endOfDay))
